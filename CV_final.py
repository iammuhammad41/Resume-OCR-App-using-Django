'''
this is need to be converted into django code.
there would be a database from where the pdf files would be saved and can be read from there.
'''
import os
import requests
import json
    ...
    if cv_file.endswith('.pdf'):
        ...
        for line in lines:
            ...
            if ':' in line:
                ...
                key = key.strip()
                value = value.strip()
                if key in ['name', 'age', 'phone', 'email', 'education', 'Experience', 'renowned', 'college', 'college_status']:
...
output_file = r'D:/CV_OCR/Json/json3.json'
